...
class Class_of_methods:
    ...
    def main_menu_clone(self, message):
        self.bot.send_message(message.from_user.id, f"📔 <b>Ты находишься в главном меню</b>\n\nВыбери действие:",  reply_markup=markup_Inline_Back1, parse_mode='HTML')
    ...
    def remember_task(self, message):
        ...
        new_time_2 = time_object + dt.timedelta(hours=1)
        new_time_3 = time_object + dt.timedelta(hours=9)
        new_time_4 = time_object + dt.timedelta(days=2)
        new_time_5 = time_object + dt.timedelta(days=6)
        new_time_6 = time_object + dt.timedelta(days=30)
        ...
        new_before_time_6 = new_time_6.strftime('%d:%H:%M')
        ...
        UnicKey_6 = self.generate_random_string(40)
        ...
        db.ACTIONS_FOR_QUEUE(action="add", queue={"UserId":f"{message.from_user.id}", "now_date":cur_time, "time_before":new_before_time_6, "data_type":"remember", "msg_text":text, "photo":photo, "UnicKey": UnicKey_6})



        self.bot.send_message(message.from_user.id, "🧠 Записал, теперь я хакну твой мозг и ты запомнтшь эту инфу навсегда", parse_mode='HTML')
        # self.bot.answer_callback_query(message.id, text=f'🧠 Записал, теперь я хакну твой мозг и ты запомнтшь эту инфу навсегда', show_alert = True)
        self.main_menu_clone(message)
    ...
    def Remind_TODAY_TASKS(self, UserId, action="send", msg=None):
        if action == "send":
            ...
            self.bot.send_message(UserId, "🥷🏿 Выбери задачи, которые ты не сделал", parse_mode='HTML', reply_markup=markup_InlineToday)
    ...
    def MORNING_TASKS(self, UserId, action="send", msg=None):
        if action == "send":
            ...
            tasks_list, count_all_tasks = db.ACTIONS_QUESTS_FOR_TODAY(str(UserId), text=None, action="get_morning_tasks")
            markup_InlineToday = types.InlineKeyboardMarkup(row_width=1)
            num = 0
            ...
            BTC_domination = Crypto.get_dominance()
            BTC, TON, TNSR = Crypto.get_cost_coins()
            self.bot.send_message(UserId, f"⭐️ <b>{day_of_week}</b> ({day} {month})\n\n<blockquote><b>📊 Bitcoin dominance: </b>{BTC_domination}\n\nBTC: {BTC}\nTON: {TON}\nTNSR: {TNSR}</blockquote>", parse_mode='HTML', reply_markup=markup_InlineToday)
